		...
		if( $address ) {
			...
			<script type="text/javascript">
				...
				function fusion_run_map_<?php echo $map_id ; ?>() {
					...
					<?php
					if( strpos( $coordinates[0]['address'], 'latlng=' ) === 0 ) {
						$coordinate_0_address = str_replace('latlng=', '', $coordinates[0]['address']);
						$latLng = explode(',', $coordinate_0_address);
						echo "var latLng = new google.maps.LatLng({$latLng[0]},{$latLng[1]});";
						$address_1_geocode = "'latLng': latLng";
					} else {
						$address_1_geocode = "'address': '" . $coordinates[0]['address'] . "'";
					}
					?>
					geocoder.geocode({ <?php echo $address_1_geocode; ?> }, function(results, status) {
						if (status == google.maps.GeocoderStatus.OK) {
							var location = new google.maps.LatLng(results[0].geometry.location.k, results[0].geometry.location.B);
							var map_options = {
								zoom: <?php echo $zoom; ?>,
								center: location,
							...
							<?php if( $smof_data['map_scrollwheel'] ): ?>
							var isDraggable = jQuery(document).width() > 640 ? true : false;
							...
							foreach( $coordinates as $key => $coordinate ):
								...
								if( strpos( $coordinate['address'], 'latlng=' ) === 0 ) {
									$coordinate['address'] = str_replace('latlng=', '', $coordinate['address']);
									$latLng = explode(',', $coordinate['address']);
									$rand_number = mt_rand();
									echo "var latLng_" . $rand_number . " = new google.maps.LatLng({$latLng[0]},{$latLng[1]});";
									$address_2_geocode = "'latLng': latLng_" . $rand_number;
									$coordinate['address'] = 'latLng';
									if( strpos( $infobox_content, 'latlng=' ) === false ) {
										$latlng_custom_infobox = true;
									} else {
										$latlng_custom_infobox = false;
									}
								} else {
									$address_2_geocode = "'address': '" . $coordinate["address"] . "'";
								}
							?>
							google.maps.event.addListenerOnce(map_<?php echo $map_id ; ?>, 'tilesloaded', function() {
								geocoder.geocode({ <?php echo $address_2_geocode; ?> }, function(results, status) {
									if (status == google.maps.GeocoderStatus.OK) {
										<?php if($coordinate['address'] == 'latLng'): ?>
										<?php if($latlng_custom_infobox == true): ?>
										var content_string = "<div class='info-window'><?php echo $infobox_content; ?></div>";
										<?php else: ?>
										var content_string = "<div class='info-window'>" + results[0].formatted_address + "</div>";
										<?php endif; ?>
										<?php else: ?>
										var content_string = "<div class='info-window'><?php echo $infobox_content; ?></div>";
										<?php endif; ?>
