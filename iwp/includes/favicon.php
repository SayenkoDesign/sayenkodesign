...
class Favicon{
	...
	public static function getFaviconsCronJob($cronStartTime){
		...
		$sitesData = DB::getArray("?:sites", "siteID, favicon, URL, lastFaviconUpdate", "1 ORDER BY siteID ASC");
	...
	public static function isFaviconFound($httpStatus,$HTMLContent){
		if ($httpStatus == 200 && (strpos($HTMLContent, "icon") !== false || strpos($HTMLContent, "shortcut icon") !== false)) {
			return true;
		} else{
			return false;
		} 
	...
	public static function storeFaviconAndDBUpdate($URL,$siteID, $lastFaviconUpdate){
	    ...
	    $result = self::storeFavicontoLocalstorage($URL, $localPath, $siteID, $lastFaviconUpdate); //to store icon into local
	  	if ($result) {
