...
if (isset($d['scheduleBackups'])) {
  if(!empty($d['scheduleBackups']) && is_array($d['scheduleBackups'])){
    foreach($d['scheduleBackups'] as $backup){ ?>
          ...
          <?php if(!empty($backup['downloadURL'])){ ?><div class="row_backup_action rep_sprite delConfHide" style="float:right;"><a class="download rep_sprite_backup" href="<?php echo $backup['downloadURL']; ?>"></a></div> <?php } ?>
