...
?>
  <tr>
    <td colspan="2">
      <div style="padding-bottom: 10px;"><a href="<?php echo $sitesName[$siteID]; ?>" style="color:#465053;font-size:13px;font-weight:bold;text-decoration:none"><?php echo $sitesName[$siteID]; ?></a></div></td>
    </tr>
    ...
    <?php if(!empty($showUpdate['coreUpdates']) && !empty($updateData['core'])){
		foreach($updateData['core'] as $type => $update){
		 ?>
   <tr>
    <td width="130" align="right" valign="top"><span style="background-color:#f15b43; color:#fff; padding:1px 5px 3px; font-size:11px; border-radius:2px; float: right;">wordpress</span></td>
    <td width="340"><?php echo 'v'. strip_tags($update['current_version']).' to '.'<a href="'.WP_CHANGELOG_URL.'Version_'.strip_tags($update['current']).' "style="color: #f2583e;">v'. strip_tags($update['current']);?></td>
  </tr>
  <?php } }
  ?>
   ...
   <?php if($i == 0){ ?>
    ...
    <span style="background-color:#f15b43; color:#fff; padding:1px 5px 3px; font-size:11px; border-radius:2px;margin-top: 8px; float: right;">plugins</span></td>
    <td width="340" style="padding-top:10px;"><?php echo strip_tags($update['name']).' - v'. strip_tags($update['old_version']).' to '.'<a href="'.WP_PLUGIN_CHANGELOG_URL.''. strip_tags($update['slug']).'/changelog/" style="color: #f2583e;">v'. strip_tags($update['new_version']); ?></td>
  </tr>
 <?php $i = 1; } 
   else{ ?>
  <tr>
    <td align="right" valign="top">&nbsp;</td>
    <td><?php echo strip_tags($update['name']).' - v'. strip_tags($update['old_version']).' to '.'<a href="'.WP_PLUGIN_CHANGELOG_URL.''. strip_tags($update['slug']).'/changelog/" style="color: #f2583e;">v'. strip_tags($update['new_version']); ?></td>
  </tr><?php }
    } 
  } ?>
  ...
  <?php if($i == 0){ ?>
   <tr>
    <td width="130" align="right" valign="top">
      <span style="background-color:#f15b43; color:#fff; padding:1px 5px 3px; font-size:11px; border-radius:2px;margin-top: 8px; float: right;">themes</span></td>
    <td width="340" style="padding-top:10px;"> <?php echo strip_tags($update['name']).' - v'. strip_tags($update['old_version']).' to '.'v'. strip_tags($update['new_version']);  ?></td>
  </tr>
  <?php $i = 1; } 
   else{ ?>
  <tr>  
    <td align="right" valign="top">&nbsp;</td>
    <td><?php echo strip_tags($update['name']).' - v'. strip_tags($update['old_version']).' to '.'v'. strip_tags($update['new_version']); ?></td>
  </tr>
  <?php }
     } 
	} 
   ...
   if(!empty($showUpdate['translationUpdates']) && !empty($updateData['translations'])){
     ?>
   <tr>
    <td width="130" align="right" valign="top"><span style="background-color:#f15b43; color:#fff; padding:1px 5px 3px; font-size:11px; border-radius:2px; float: right;">translations</span></td>
 ...
 <?php  }
	...
	<tr>
    <td width="130" align="right" valign="top">
      <span style="background-color:#f15b43; color:#fff; padding:1px 5px 3px; font-size:11px; border-radius:2px; float: right;">wordpress</span></td>
