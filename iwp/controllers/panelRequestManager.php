...
class panelRequestManager{
	...
	public static function getSitesUpdates($userID = '', $doCache = true){
		...
		return array('siteView' => $siteView, 'pluginsView' => $pluginView, 'themesView' => $themeView, 'translationsView' => $translationsView, 'coreView' => $coreView, 'siteViewCount' => $siteViewCount, 'totalUpdateCount' => $totalUpdateCount, 'lastReloadTime' => $lastReloadTime, 'updatePageEmailCronReqNotification' => $updatePageEmailCronReqNotification);
	}
	...
	public static function getSites(){
		...
		if(function_exists('multiUserGetSites')){
			...
			$sitesData = DB::getArray("?:sites", "siteID, URL, adminURL, name, IP, WPVersion, adminUsername, isOpenSSLActive, network, parent, httpAuth, callOpt, connectURL, connectionStatus, links, notes, favicon, lastFaviconUpdate ", "1 ORDER BY name", "siteID");
		}
		...
		if(!empty($clientPluginUpdate)){
			...
			if(!empty($clientPluginUpdate['newVersion']) && !empty($clientPluginUpdate['updateDetails'][$clientPluginUpdate['newVersion']]['downloadLink'])){
				if(empty($clientUpdates['clientUpdateVersion']) || (!empty($clientUpdates['clientUpdateVersion']) && version_compare($clientUpdates['clientUpdateVersion'], $clientPluginUpdate['newVersion']) == -1)){
					...
					$clientUpdates['clientUpdatePackage'] = $clientPluginUpdate['updateDetails'][$clientPluginUpdate['newVersion']]['downloadLink'];			
