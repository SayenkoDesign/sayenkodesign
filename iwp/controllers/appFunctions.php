...
function getSiteData($siteID, $byPassAccess = NULL){
	if($siteID){
...
function getSitesData($siteIDs=false, $where='1'){ //for current user
	if(userStatus() == "admin"){
		...
		return DB::getArray("?:user_access UA, ?:sites S", "S.*, UA.siteID", "UA.userID ='".DB::esc($GLOBALS['userID'])."' AND S.siteID=UA.siteID", "siteID");
	}
}
...
function checkTriggerStatus(){
	...
	$data = DB::getArray("?:history", "siteID, historyID", $where);
...
function showBrowserCloseWarning(){
	$where = array(
			      		'query' =>  "H.status = ':status'",
			      		'params' => array(
			               ':status'=>'multiCallWaiting'
           				)
        			);
	$isMultiCallJobActive = DB::getExists("?:history H", "H.historyID", $where);
